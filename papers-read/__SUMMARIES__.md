...
: We want a differential loss function to train a model, but we also want to apply reasoning (i.e. constraints) to the output, which destroys the differentiability of the deep learner. So we contrive a "semantic loss function": the semantic loss is a function of the probability of sampling a state that satisfies the constraint. (The semantic loss term can just be added to any existing loss function.) Three examples of semantic losses which are successfully applied in semi-supervised experiments are one-hot constraint, ranking constraint, and path constraint. *(code available)*
<!--stackedit_data:
eyJoaXN0b3J5IjpbLTMyODExMjAyNCwxNzIyMzIyOTAzLDEyNz
